...
def run(cfg: ModulusConfig) -> None:
    ...
    # start solver
    slv.solve()
    Post_processing(solid_heat_net, fluid_heat_net)
...
def Post_processing(solid_heat_net, fluid_heat_net):

    # Get solid coordinate
    plot_num = 201
    x = torch.linspace(chip_pos, chip_pos + chip_width, plot_num)
    y = torch.linspace(-0.5, -0.5 + chip_height, plot_num)
    X, Y = torch.meshgrid(x, y)
    x_solid, y_solid = X.reshape(-1, 1).cuda(), Y.reshape(-1, 1).cuda()
    inputs = {"x": x_solid, "y": y_solid}
    outputs_solid = solid_heat_net(inputs)
    T_predict = outputs_solid["theta_s_star"].cpu().detach().numpy().flatten() + 170.0 # Normalize
    x_plot, y_plot = x_solid.cpu().detach().numpy().flatten(), y_solid.cpu().detach().numpy().flatten()
    Plot_solid(x_plot, y_plot, T_predict, f"PINN_solid", 3)
    ...
    outputs_l = fluid_heat_net(inputs)
    T_fluid_l = outputs_l["theta_f_star"] + 70.0 # Normalize
    plot_l = (x_fluid_l, y_fluid_l, T_fluid_l)
    ...
    outputs_m = fluid_heat_net(inputs)
    T_fluid_m = outputs_m["theta_f_star"] + 70.0 # Normalize
    plot_m = (x_fluid_m, y_fluid_m, T_fluid_m)
    ...
    outputs_r = fluid_heat_net(inputs)
    T_fluid_r = outputs_r["theta_f_star"] + 70.0 # Normalize
    plot_r = (x_fluid_r, y_fluid_r, T_fluid_r)

    T_l = T_fluid_l.cpu().detach().numpy()
    T_m = T_fluid_m.cpu().detach().numpy()
    T_r = T_fluid_r.cpu().detach().numpy()

    T_all = np.concatenate([T_l, T_m, T_r])
    np.savetxt(f"Height{chip_height}_T.txt", T_all)

    Plot_fluid(plot_l, plot_m, plot_r, f"PINN_fluid", 1)

def Plot_solid(X, Y, u_plot, name, flag=3):
    print(f"Plot prediction {name} in 2D contour")
    plt.clf()
    fig, axes = plt.subplots(1, 1, figsize=(chip_width, chip_height))
    if flag == 1:
        value_min, value_max = 133, 165
    else:
        value_min, value_max = np.min(u_plot), np.max(u_plot)
    t1 = axes.tricontourf(X, Y, u_plot, cmap='coolwarm', levels=np.linspace(value_min, value_max,101))
    x_ticks = np.round(np.linspace(np.min(X), np.max(X), 3), 1)  # 5 個 ticks, 1 位小數
    y_ticks = np.round(np.linspace(np.min(Y), np.max(Y), 2), 1)
    axes.set_xticks(x_ticks)
    axes.set_yticks(y_ticks)
    axes.tick_params(axis='both', which='major', labelsize=8)
    # axes.set_xlabel(r'$x$', fontsize=12)
    # axes.set_ylabel(r'$y$', fontsize=12)
    bar = fig.colorbar(t1)
    ticks = np.linspace(value_min, value_max, 2)
    bar.set_ticks(ticks)
...
def Plot_fluid(plot_l, plot_m, plot_r, name, flag=3):
    ...
    if flag == 1:
        value_min, value_max = 20, 163
    else:
        value_min, value_max = np.min(T_l), np.max(T_l)
    t1 = axes.tricontourf(X_l, Y_l, T_l, cmap='coolwarm', extend="both", levels=np.linspace(value_min, value_max,101))
    t2 = axes.tricontourf(X_m, Y_m, T_m, cmap='coolwarm', extend="both", levels=np.linspace(value_min, value_max,101))
    t3 = axes.tricontourf(X_r, Y_r, T_r, cmap='coolwarm', extend="both", levels=np.linspace(value_min, value_max,101))
    # axes.set_xlabel(r'$x$', fontsize=20)
    # axes.set_ylabel(r'$y$', fontsize=20)
    # bar = fig.colorbar(t1)
    # ticks = np.linspace(value_min, value_max, 5)
    # bar.set_ticks(ticks)
    # bar.set_ticklabels([f'{int(tick)}' for tick in ticks])
    plt.savefig(f'ldc_{name}.png', dpi=500, bbox_inches='tight', transparent=True)
    mpl.pyplot.close()
    return
